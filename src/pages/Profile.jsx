...
import { setEditProfileUpdatedFalse } from '../store/auth_store';
...
export default function ProfileScreen() {
  ...
  const { user, edit_profile } = useSelector((state) => state.authSlice);
  ...
  useEffect(() => {
    if (localStorage.getItem('image_url')) {
      ...
      if (image_url !== null && image_url !== '' && image_url !== undefined && image_url !== 'undefined' && image_url !== 'null') {
        setPreviewUrl(image_url);
      }
      // setPreviewUrl(localStorage.getItem('image_url'));
    }

  }, [dispatch]);

  useEffect(() => {
    if (edit_profile.edit_message) {
      setTimeout(() => {
        dispatch(setEditProfileUpdatedFalse());
      }, 2000);
    }
  }, [edit_profile.edit_message]);
  ...
  const handleImageUpload = async (file) => {
    ...
    try {
      const result = await dispatch(AuthService.update_user_image(formData)).unwrap();
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
      ...
      {edit_profile.edit_message &&
        <MsgBox
          ...
          type={edit_profile.profile_updated ? 'success' : 'error'}
          visible={true}
        />
        ...
        <div className="bg-white rounded-3xl shadow-xl p-8 mb-8 border border-gray-100 relative">
          ...
          <div className="flex flex-col lg:flex-row items-center lg:items-start space-y-6 lg:space-y-0 lg:space-x-8">
            ...
            <div className="flex-shrink-0 relative">
              <div className="relative w-24 h-24 bg-gradient-to-r from-blue-500 to-purple-500 rounded-3xl flex items-center justify-center shadow-lg overflow-hidden">
                ...
                <MdModeEditOutline
                  ...
                  disabled={edit_profile.edit_pending}
                  className={`absolute bottom-2 right-2 text-gray-600 hover:text-gray-800 text-2xl cursor-pointer ${edit_profile.edit_pending ? 'opacity-50 cursor-not-allowed' : ''
                    }`}
                />
              </div>
            ...
            {/* <div className="flex-shrink-0">
              <div className="relative w-24 h-24 bg-gradient-to-r from-blue-500 to-purple-500 rounded-3xl flex items-center justify-center shadow-lg">
                <IoPerson className="text-white text-3xl" />
                <MdModeEditOutline
                  onClick={() => {
                    console.log('Edit image clicked')
                  }}
                  className='absolute bottom-2 right-2 text-gray-600 hover:text-gray-800 text-2xl cursor-pointer' />
              </div>
            </div> */}
            ...
            <div className="flex-1 text-center lg:text-left">
              <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-4">
                ...
                {/* Premium Badge */}
                <div className={`inline-flex items-center space-x-2 px-4 py-2 rounded-2xl mt-4 lg:mt-0 ${user?.is_premium
